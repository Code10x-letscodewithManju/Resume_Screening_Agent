...
# from agentic.config import DATA_DIR, UPLOAD_DIR, DEFAULT_WEIGHTS
# from agentic.graph import build_agent_graph, AgentState
# from agentic.schemas import CandidateResult, JD, ResumeParsed
# from agentic.reporting import build_candidate_report_pdf  #just added for reporting feature
...
#     semantic_w = st.slider("Semantic weight", 0.0, 1.0, DEFAULT_WEIGHTS["semantic"], 0.05)
#     exp_w = st.slider("Experience weight", 0.0, 1.0, DEFAULT_WEIGHTS["experience"], 0.05)
#     outcome_w = st.slider("Outcome weight", 0.0, 1.0, DEFAULT_WEIGHTS["outcome"], 0.05)
#     risk_w = st.slider("Risk penalty weight", 0.0, 1.0, DEFAULT_WEIGHTS["risk"], 0.05)
...
#             st.write(f"Experience: {jd.min_years_experience}–{jd.max_years_experience} years")
#             if jd.locations:
#                 st.write(f"Preferred locations: {', '.join(jd.locations)}")
#             st.write(f"Employment type: {jd.employment_type}")
#             st.markdown("**Must-have skills:**")
#             st.write(", ".join(jd.must_have_skills) if jd.must_have_skills else "Not detected")
#             st.markdown("**Nice-to-have skills:**")
#             st.write(", ".join(jd.nice_to_have_skills) if jd.nice_to_have_skills else "Not detected")
...
#             st.markdown("**Risk flags from JD (potential bias / unrealistic asks):**")
#             if jd.risk_flags:
...
#             st.caption("Positive RankDelta = candidate moved down when PII removed; negative = moved up.")

#         # Candidate cards
#         st.header("Step 5 · Candidate Cards (Reasoning & Actions)")
#         for c in full_results:
#             s = c.scores
#             must_hits = getattr(s, "must_have_hits", [])
#             must_miss = getattr(s, "must_have_miss", [])
...
# if bias_clicked:
#     st.info("Run the screening first; fairness uses the parsed JD and resumes from the last run.")
